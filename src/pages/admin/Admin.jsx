...
function App()
{
	...
	let [ob1,setob1]=React.useState({name:"",email:"",phone:"",password:""})
	let [ob2,setob2]=React.useState({id:0,name:"",email:"",phone:"",password:""})
	...
	const update=e=>{
		e.preventDefault()
		_patch("admin/"+ob2.id,ob2)
		.then(d=>admin.map(x=>x.id===ob2.id?ob2:x))
		.then(d=>dispatch({type:"admin",payload:d}))
	}
	const del=id=>{
		_delete("admin/"+id)
		.then(d=>admin.filter(x=>x.id!==id))
		.then(d=>dispatch({type:"admin",payload:d}))
	}
	const edit=x=>{
		setob2(x)
